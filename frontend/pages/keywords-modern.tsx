...
export default function KeywordsModern() {
  ...
  const getTypeLabel = (type: Keyword['type']) => {
    return type === 'include' ? t('include') || 'شامل' : t('exclude') || 'مستثنی'
  }
  ...
  const typeFilters = [
    { key: 'all' as const, label: t('all') || 'همه', count: keywords.length },
    { key: 'include' as const, label: t('include') || 'شامل', count: keywords.filter(k => k.type === 'include').length },
    { key: 'exclude' as const, label: t('exclude') || 'مستثنی', count: keywords.filter(k => k.type === 'exclude').length }
  ]

  const statusFilters = [
    { key: 'all' as const, label: t('all') || 'همه', count: keywords.length },
    { key: 'active' as const, label: t('active') || 'فعال', count: keywords.filter(k => k.status === 'active').length },
    { key: 'inactive' as const, label: t('inactive') || 'غیرفعال', count: keywords.filter(k => k.status === 'inactive').length }
  ]

  return (
    <Layout title={t('keywordManagement') || 'Keyword Management'}>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
              {t('keywordManagement') || 'مدیریت کلیدواژه‌ها'}
            </h1>
            <p className="text-gray-600 dark:text-gray-400">
              {t('keywordFilterDescription') || 'تنظیم فیلترهای کلیدواژه برای فوروارد خودکار'}
            </p>
          </div>
          <ModernButton
            variant="primary"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-6">
            ...
            <ModernCard variant="glass">
              ...
              <ModernInput
                placeholder={t('searchKeywordPlaceholder') || 'جستجو کلیدواژه...'}
                value={searchTerm}
          ...
          <div className="lg:col-span-3">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                <LoadingSpinner size="lg" text={t('loadingKeywords') || 'در حال بارگذاری کلیدواژه‌ها...'} />
              </div>
            ) : (
              <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {keyword.caseSensitive && (
                              <span className="px-2 py-1 rounded text-xs font-medium text-orange-600 bg-orange-500/10">
                                {t('caseSensitive') || 'حساس به حروف'}
                              </span>
                            )}
                          </div>
                          <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                            <span>{t('matchCount')}: {keyword.matchCount.toLocaleString('fa-IR')}</span>
                            <span>{t('lastUpdated')}: {keyword.updatedAt}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          title={keyword.status === 'active' ? t('deactivate') || 'غیرفعال کردن' : t('activate') || 'فعال کردن'}
                        >
                          {keyword.status === 'active' ? (
                ...
                {filteredKeywords.length === 0 && (
                  <ModernCard variant="glass" className="text-center py-12">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      {t('noKeywordsToShow') || 'کلیدواژه‌ای برای نمایش وجود ندارد'}
                    </p>
                  </ModernCard>
                )}
              </div>
      ...
      <ModernModal
        isOpen={showAddModal}
        onClose={closeModal}
        title={editingKeyword ? t('editKeyword') || 'ویرایش کلیدواژه' : t('addNewKeyword') || 'افزودن کلیدواژه جدید'}
      >
        <div className="space-y-4">
          <ModernInput
            label={t('keywordText') || 'متن کلیدواژه'}
            placeholder={t('keywordTextPlaceholder') || 'کلیدواژه یا عبارت مورد نظر'}
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-sm font-medium">{t('include') || 'شامل'}</div>
                <div className="text-xs opacity-75">{t('includeKeywordDescription') || 'پیام‌ها با این کلیدواژه'}</div>
              </button>
              <button
              ...
              >
                ...
                <div className="text-sm font-medium">{t('exclude') || 'مستثنی'}</div>
                <div className="text-xs opacity-75">{t('excludeKeywordDescription') || 'پیام‌ها بدون این کلیدواژه'}</div>
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded"
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">{t('caseSensitive') || 'حساس به حروف کوچک و بزرگ'}</span>
            </label>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {editingKeyword ? t('update') || 'به‌روزرسانی' : t('add') || 'افزودن'}
            </ModernButton>
            <ModernButton
              variant="secondary"
              onClick={closeModal}
