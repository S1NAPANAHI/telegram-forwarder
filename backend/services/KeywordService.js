...
class KeywordService {
  async addKeyword(userId, keywordData) {
    ...
    if (!canAdd) throw new Error('Keyword limit reached. Please upgrade your plan.');
  ...
  async findMatchingKeywords(userId, text) {
    ...
    for (const keyword of keywords) {
      if (this.doesTextMatchKeyword(text, keyword)) matches.push(keyword);
    }
    return matches;
  }
  ...
  doesTextMatchKeyword(text, keyword) {
    ...
    if (mode === 'exact' || keyword.exact_match) return searchText === searchKeyword;
