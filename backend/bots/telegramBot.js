  ...
  registerPassiveDiscovery() {
    const upsertDiscovered = async (chat, userId = null) => {
      try {
        ...
        // If we have a user context, save to discovered_chats with user_id
        if (userId) {
        ...
        // Also save via ChatDiscoveryService for backward compatibility
        ...
        console.warn('Passive discovery error:', e.message); 
      }
    };
  ...
  setupCommandHandlers() {
    ...
    this.bot.onText(/^\/help\b/i, async (msg) => {
      try {
        ...
        try {
          await this.bot.sendMessage(msg.chat.id, '🆘 Available commands: /start /help /status /webapp /discover /ping');
        } catch {}
      }
    });
    ...
    this.bot.onText(/^\/status\b/i, async (msg) => {
      try {
        ...
        try {
          await this.bot.sendMessage(msg.chat.id, `📊 Bot Status: ${this.monitoredChannels.size} monitored channels`);
        } catch {}
      }
    });
    ...
    this.bot.onText(/^\/webapp\b/i, async (msg) => {
      try {
        ...
        try {
          await this.bot.sendMessage(msg.chat.id, `🌐 Web App: ${WEBAPP_URL}`);
        } catch {}
      }
    });
    ...
    this.bot.onText(/^\/discover\b(?:\s+(?<handle>@?[A-Za-z0-9_]+))?/i, async (msg, match) => {
      try {
        ...
        // Send additional info about dashboard
        if (chats.length > 0) {
          const keyboard = {
            inline_keyboard: [
              [{ text: '🌐 Open Dashboard', web_app: { url: `${WEBAPP_URL}?tab=channels` } }]
            ]
          };
          await this.bot.sendMessage(
            msg.chat.id, 
            '💡 Visit your dashboard to configure monitoring for discovered chats:',
